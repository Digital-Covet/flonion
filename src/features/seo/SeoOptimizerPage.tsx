...
function StatCard(props: { stat: KpiStat }) {
  return (
    <div class="flex items-center gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div
        class={`flex size-12 items-center justify-center rounded-xl ${props.stat.accent ?? "bg-slate-100 text-slate-600"}`}
      >
        ...
        <Show when={props.stat.trend}>
          <span
            class={`flex items-center gap-1 text-[11px] font-semibold ${
              props.stat.trend!.direction === "positive"
                ? "text-blue-600"
    ...
    <>
      ...
      <div class="mx-auto max-w-7xl space-y-6">
        ...
        <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <For each={seoKpiStats}>{(stat) => <StatCard stat={stat} />}</For>
        </div>
