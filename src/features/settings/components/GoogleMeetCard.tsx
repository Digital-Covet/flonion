      ...
      <div>
        ...
        <div class="mb-4 flex items-start justify-between gap-3">
          <div class="flex items-center gap-3">
            <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-control bg-secondary/10 text-secondary shadow-xs">
              <Video size={22} />
            </div>
            <div>
        ...
        >
          <div class="space-y-3.5">
            {/* Features Enabled */}
            <div class="space-y-2 rounded-card border border-border/60 bg-muted/30 p-3">
              ...
              <div class="flex items-center justify-between text-xs">
                <span class="flex items-center gap-2 text-foreground font-medium">
                  <Users size={14} class="text-secondary" />
                  Team Meetings
                </span>
                <span class="inline-flex items-center gap-1 text-xs font-medium text-success">
                  <CheckCircle2 size={12} />
            ...
            <div class="pt-1">
              <div class="flex items-center justify-between">
                ...
                <button
                  ...
                  class="inline-flex items-center gap-1.5 rounded-control bg-secondary px-2.5 py-1 text-xs font-medium text-secondary-foreground hover:bg-secondary/80 transition-colors disabled:opacity-50"
                >
                  {generatingLink() ? (
                    <Loader2 size={13} class="animate-spin" />
                  ) : (
                    <Sparkles size={13} class="text-star" />
                  )}
              ...
              <Show when={testLink()}>
                <div class="mt-2.5 flex items-center gap-2 rounded-card border border-success/30 bg-success-muted p-2.5 text-xs">
                  <span class="flex-1 truncate font-mono text-foreground select-all">
                    {testLink()}
                  </span>
                  <button
                    type="button"
                    onClick={handleCopyLink}
                    class="inline-flex shrink-0 items-center gap-1 rounded-control bg-background px-2 py-1 text-xs font-medium border border-border hover:bg-muted text-foreground transition-colors"
                  >
                    {copied() ? (
                      <>
                  ...
                  <a
                    ...
                    class="inline-flex shrink-0 items-center gap-1 rounded-control bg-secondary px-2 py-1 text-xs font-medium text-secondary-foreground hover:bg-secondary/80 transition-colors"
                  >
                    <span>Join</span>
              ...
              <Show when={generateError()}>
                <div class="mt-2 rounded-card border border-destructive/20 bg-destructive-muted p-2 text-xs text-destructive">
                  {generateError()}
                </div>
              </Show>
            </div>
