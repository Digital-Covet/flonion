...
export const ForgotPasswordForm: Component<ForgotPasswordFormProps> = (
  ...
  const handleSubmit = async (event: SubmitEvent) => {
    ...
    try {
      ...
      // Network failure must not look like "nothing happened" — the route
      // still shows the generic "check your email" state on success (no
      // account enumeration), but a transport error is reported honestly.
      setErrorMessage(
        authErrorMessage(e, "Couldn't reach the server. Please try again."),
      );
      setStatus("idle");
    }
  };
