...
import { ExternalLink, Inbox, PenSquare, Share2 } from 'lucide-solid'
      ...
      <For each={props.links}>
        {(link) => {
          ...
          const linkClasses = cn(
            'flex flex-col items-center gap-2 rounded-xl border border-slate-200 p-4 text-center transition-all hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-700',
            'text-slate-600'
          )

          const content = (
            <>
              <div class="flex size-10 items-center justify-center rounded-full bg-slate-100 text-slate-500 transition-colors group-hover:bg-emerald-100 group-hover:text-emerald-600">
                <Icon size={18} />
              </div>
